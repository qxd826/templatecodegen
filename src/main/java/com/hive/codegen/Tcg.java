...
public class Tcg extends org.eclipse.swt.widgets.Composite {
    ...
    private Button btnTruncateTablePrefix;
    private Button      rdoSrcDir;
    private Button      rdoSrcType;
        ...
        try {
            ...
            }
            {
                ...
                sashForm1.setLayoutData(sashForm1LData);
                {
                    ...
                    {
                        btnTruncateTablePrefix = new Button(composite1, SWT.CHECK | SWT.LEFT);
                        FormData truncateTablePrefixLData = new FormData();
                        truncateTablePrefixLData.left =  new FormAttachment(0, 1000, 228);
                        truncateTablePrefixLData.top =  new FormAttachment(0, 1000, 0);
                        truncateTablePrefixLData.width = 110;
                        truncateTablePrefixLData.height = 17;
                        btnTruncateTablePrefix.setLayoutData(truncateTablePrefixLData);
                        btnTruncateTablePrefix.setText("\u622a\u6389\u8868\u524d\u7f00");
                    }
                    {
                        ...
                        cmpValues.setText("");
                        cmpValues.setFont(SWTResourceManager.getFont("Courier", 12, 0, false, false));
                ...
                }
                {
                    sashForm2 = new SashForm(sashForm1, SWT.VERTICAL | SWT.V_SCROLL);
                    {
                        ...
                        {
                            rdoSrcDir = new Button(composite2, SWT.RADIO | SWT.LEFT);
                            FormData rdoSrcDirLData = new FormData();
                            rdoSrcDirLData.left = new FormAttachment(0, 1000, 180);
                            rdoSrcDirLData.width = 45;
                            rdoSrcDirLData.height = 16;
                            rdoSrcDir.setLayoutData(rdoSrcDirLData);
                            rdoSrcDir.setText("\u76ee\u5f55");
                        }
                        {
                            rdoSrcType = new Button(composite2, SWT.RADIO | SWT.LEFT);
                            FormData rdoSrcTypeLData = new FormData();
                            rdoSrcTypeLData.left = new FormAttachment(0, 100, 114);
                            rdoSrcType.setLayoutData(rdoSrcTypeLData);
                            rdoSrcType.setText("\u6587\u4ef6");
                            rdoSrcType.setSelection(true);
                        }
                        {
                        ...
                        }
                        {
                            ...
                            cmpTemplate.setText("");
                            cmpTemplate.setFont(SWTResourceManager.getFont("Courier", 12, 0, false, false));
                    ...
                    }
                    {
                        ...
                        composite3.setLayout(composite3Layout);
                        {
                            ...
                            cmpOutput.setFont(SWTResourceManager.getFont("Courier", 12, 0, false, false));
                            cmpOutput.setText("");
    ...
    private void btnTemplateFromFileMouseUp(MouseEvent evt) {
        String newValues = "";
        if (rdoSrcDir.getSelection()) {
            File file = new File("templates");
            dirDialog.setFilterPath(file.getAbsolutePath());
            String rootDir = dirDialog.open();
            // String rootDir="D:\\work\\workspace\\templateCodeGen\\templates";
            // dirDialog = null;
            if (rootDir != null) {
                StringBuilder bd = new StringBuilder();
                bd.append(rootDir).append(GenEngine.RT);
                getAllFileNamesInDir(bd, rootDir, rootDir);
                newValues = bd.toString();
            }
        } else {
            newValues = getStringFromFile();
        }
        if (newValues != null) {
    ...
    private void getAllFileNamesInDir(StringBuilder bd, String dir, String rootDir) {
        // int rootDirLen = rootDir.length();
        // bd.append(dir).append(RT);
        for (File file : new File(dir).listFiles()) {
            if (file.isFile()) {
                bd.append(file.getAbsolutePath()).append(GenEngine.RT);
            } else {
                getAllFileNamesInDir(bd, file.getAbsolutePath(), rootDir);
            }
        }
    }
    ...
    private void btnRendMouseUp(MouseEvent evt) {
        ...
        engine.setTruncateTablePrefix(btnTruncateTablePrefix.getSelection());
        String result = null;
        try {
            result = engine.render(cmpValues.getText(), cmpTemplate.getText(), rdoSrcDir.getSelection());
        } catch (Exception e) {
            result = e.getMessage();
        }
